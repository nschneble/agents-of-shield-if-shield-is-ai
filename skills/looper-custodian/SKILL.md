...
- **Body** mirrors the phases: A reaped (info) / B proposals (checkboxes) / C digest (info) / E research (info + any checkboxes). When E was skipped by the usage-window gate, its line reads `E: deferred — usage window at N%, resets ~HH:MM; finish with /looper-custodian resume <date>` instead of a digest — the report still ships on C/A/B.
...
- Does NOT dispatch Phase E into a near-exhausted usage window — it probes the real `anthropic-ratelimit-unified-*` window before E and defers (reports C/A/B + resume breadcrumb) at ≥95%, never fabricating a pause or a percent when the probe can't read it.
