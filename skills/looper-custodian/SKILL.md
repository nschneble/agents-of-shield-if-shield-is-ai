...
**The rule carries a check, and the check reads the LOG.** A rule addressed to an unattended `claude -p` cron with nobody watching is recall, and recall is not enforcement (`scripts/correction-gates/README.md`). So `scripts/custodian-phase-order.sh` reads a run's `custodian-log.jsonl` and exits 1 when a `phase:"E"` line lands ahead of a `phase:"B"` line inside the same run segment — segments split at a `resume` marker, so a resume tail is judged on its own terms, the way the rule binds it. **Phase F runs it over the run's own log before writing the report, and the report carries its verdict line.** Be exact about what a clean result buys: it asserts the order the run LOGGED and nothing further — a run that dispatched out of order and logged in order passes it (decision 24, `docs/looper-custodian.md`). What it buys is that a broken Monday is visible the same week instead of whenever someone next reads a log by hand. A line with no parseable `phase` key, and a segment holding no B line or no E line, are reported separately and are never violations: an archived log is old, not pre-schema, and a check that floods false positives on history gets ignored. `scripts/custodian-phase-order.test.sh` is its both-directions test.
...
- `scripts/custodian-phase-order.sh` — Phase F's log-order check: P1, no `phase:"E"` line ahead of a `phase:"B"` line in one run segment. Asserts log order only, never runtime serialization (`scripts/custodian-phase-order.test.sh` is its both-directions test). Pure `jq`, no external store.
