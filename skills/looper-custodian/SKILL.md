...
- **Fingerprint pre-filter, before dispatch.** `scripts/custodian-fingerprint-cache.sh diff` sha256-checks each `file:line` citation against its last confirmed hash; `UNCHANGED` ones skip re-audit but still count `citations_resolved`, `NEW`/`CHANGED`/`GONE` still go to the LLM fan-out. `GONE` is never a shortcut to `B-retire`. Scope and mechanics: `references/phase-detail.md` `## Phase B`.
