---
name: looper-defend
description: On-demand autonomous vulnerability hunt + remediation over a whole repo. Trigger when the user says "find and fix bugs", "find and fix vulnerabilities", "security scan this repo", "hunt for vulns", or "run looper-defend".
---
...
1. **Run `looper-plan` for the finding** — produce the standard seven-section brief, scoped down to the single remediation. (A finding-fix is a small wave, so the brief is brief; but a rung MUST be named or `looper-build` STOPs, and the mechanized-prediction pass is where the repro oracle gets dry-run.)
   ...
   - **Auth/authz remediation commonly — but not always — trips plan's escalation.** Find explicitly hunts missing-authz / broken-access-control classes (`## Find`). `looper-plan`'s escalation trigger is narrower than "any auth code touched": it fires on a **public API contract change** (`## Escalation to specialist` → "public API contract changes (auth surface, extension API)"), and `looper-build`'s security-review row fires only WHEN plan already escalated (its table is headed "specialist gates relevant when plan ESCALATE" — an orchestrator job downstream of plan's `ESCALATE`, not an independent build-time mandate). So two cases:
     - **The fix changes the public auth surface** (a new/removed endpoint, a changed token or permission contract, an altered extension API) → plan emits `ESCALATE: security-review`, and defend routes it the normal way: it does NOT swallow the escalation or assume build runs straight through — it hands the escalation to the orchestrator, which fires the specialist, appends its output as `gate outputs`, and re-dispatches the wave at build (the normal `ESCALATE` flow, `docs/looper-framework.md` → plan-absorbs-deterministic note). Under a bare `apply` with no orchestrator present to fire the gate, defend STOPs that finding and reports the gate it needs — never a silently-substituted security judgment.
...
The expected set is the adapter's dep-bump path pair — the manifest + lockfile for the matched stack (e.g. `package.json` + `package-lock.json` for Node, `Gemfile` + `Gemfile.lock` for Ruby, `go.mod` + `go.sum` for Go — the `## Stack detection + adapter table`'s auto-patch column). Then gate:

- **Every changed path is in that set** → the diff is manifest/lockfile-confined as predicted; proceed to `looper-commit` and the bump lands as a reviewable draft-PR commit.
- **ANY changed path falls outside it** → build touched application code the triage-time classification could not foresee, so the `diff-touches-app-code` demotion (`## Governing principle`) fires FOR REAL: STOP the auto-path, do NOT commit, and demote the finding to propose-only — it re-enters the report as a normal ticked-to-apply `P-<n>` awaiting a human tick, exactly as if it had never been auto-eligible.

This post-build diff check is the enforcement the triage-time eligibility flag only predicted. Every auto-applied bump that DOES clear the gate and land is reported explicitly in the run report so the present human sees it — draft-PR commit, reversible.
...
- `looper-plan` / `looper-build` / `looper-verify` / `looper-review` / `looper-commit` — the remediation pipeline, the full per-wave chain. Defend runs plan to build the wave brief and routes; it never patches directly, and when an auth-finding fix changes the public auth surface, the resulting plan-stage `ESCALATE: security-review` is handled the normal way (orchestrator fires the specialist, appends `gate outputs`, resumes at build). A fix that only tightens an existing check may not trip that escalation on its own — plan owns the trigger — so defend notes the authz class in the report instead of manufacturing a gate (`## Patch` → the two-case auth handling).
