...
Build proceed when `looper-plan` brief in hand. Plan absorbs the deterministic portion of pre-build gates (mechanized contract tests, Squawk dry-run, caller-graph grep, baseline measurement); build consumes the brief direct. Two cases for specialist gate output:
...
5. Smallest code change satisfy spec. No bonus refactors, no unrelated cleanups, no defensive code for hypothetical futures. Three similar lines beat premature abstraction. Building for an unarrived need spends optionality + eats NPV, which cheap codegen refunds neither (see `looper-plan` rung 1); defer it to the wave with the real need.
