...
**Brief authoring — three standing quality instructions that pre-empt the most common corrective wave.** When runtime code is built to a cleared pre-build contract it tends to ship *correct* on the first build; the corrective waves that still fire are overwhelmingly NOT code defects but **untested-invariant**, **stale-docstring**, and **orphaned-prose** gaps a crew catches after the fact. All three are cheaper to prevent in the originating wave than to close with a doc/test-only corrective wave plus a re-crew. So every wave brief that ships runtime code carries these standing lines (the third fires only on deletion/removal waves):

- **Test any NEW cross-layer or dynamic invariant this wave introduces — in BOTH directions if it's a toggle.** A wave that adds a nested-tablist isolation (outer arrows must not reach inner tabs), a predicate that flips an attribute on/off (a panel that gains `tabIndex` when empty and must *re-drop* it when filled), or any state-machine edge tends to ship the forward assertion and omit the reverse/isolation one — which the chemist then flags as a blocker. Name the invariant in the brief and require its full test (round-trip, not one-way). Observed twice in one run (K1 isolation wave 1, dynamic-tabIndex round-trip wave 3), each costing a corrective wave + re-crew.
- **Reconcile the TOP-OF-FILE docstring, not just inline comments, when the change invalidates a file's stated invariant.** A wave that makes a previously-unconditional behavior conditional (e.g. "the panel is always `tabIndex={0}`" → "only when it holds no focusable descendant") reliably updates the inline comment at the change site but leaves the overview docstring asserting the old invariant — now self-contradicting the file's own new comments. The chronicler flags exactly this as doc-drift. Instruct the-looper to grep the file's header docstring for any claim the change falsifies. Observed as the final-crew blocker of this run.
- **On a DELETION / feature-removal wave, sweep the WHOLE repo for prose that references the deleted symbol or feature — not just the edited files.** The docstring-reconcile line above covers files the wave *edits*; a deletion's stale-reference blast radius extends to files it NEVER touches — consumer UI copy, a sibling empty-state panel's instructions, backend `@ApiResponse`/DTO descriptions, DB-schema comments, READMEs — all still describing the removed feature. This is the single costliest recurring corrective-wave cause on removal goals. Brief the-looper to `grep -ri` the deleted feature's name AND its public symbols across the whole repo (both apps in a monorepo) and reconcile every hit, then re-run the mechanical grep at the end to prove none survive. The most dangerous class is **rendered user-facing copy**: a removal can leave an instruction promising a control that no longer exists (SC 3.3.2), which ships green because its test still asserts the stale string. Observed as this run's interim-crew BLOCKER — deleting a live "try it out" form left a WelcomePanel telling users to "hit Send" on a form that was gone, plus ~15 other stale doc/comment/description surfaces across both apps, costing a full doc-only corrective wave + 6-agent re-crew. See `[[feedback-deletion-blast-radius-doc-drift]]`.

These are prevention, not new scope — they cover the wave's OWN change (and, for a deletion, the references the wave's own removal orphaned), same as the "delete dead code your change orphans" carve-out. A tightly-scoped "implement the contract, nothing beyond" brief is what suppresses them, so the brief must name them explicitly as in-scope.

**Gate authoring on a deletion wave — point the specialist at the CONSUMERS, not just the deletion site.** A pre-build specialist gate (2b) whose brief names only the files being deleted/edited gives the specialist no line of sight to a regression in a file *outside* that set. On a feature-removal wave the gate brief must additionally name the deleted feature's consumers, callers, and adjacent user-facing copy so the specialist reviews them for now-broken references. Observed this run: the a11y-lead pre-build gate cleared the Try-It deletion cleanly, but the WelcomePanel content regression (stale "hit Send" instructions) escaped it because WelcomePanel sat outside the gate's named fileset — the interim crew caught it instead, one round later than necessary. A deletion gate that lists "…and every consumer/adjacent panel that referenced this feature" closes that blind spot up front.
