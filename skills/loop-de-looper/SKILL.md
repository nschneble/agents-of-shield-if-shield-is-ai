...
├── final crew pass (all seven)          → before declaring goal-complete
...
**2d. Crew trigger check.** After every wave, evaluate:

- `waves_since_crew >= 4` OR
- `cumulative_files_changed >= 30` OR
- `batched_findings` grew by 5 or more since the last crew pass — the batch filling fast is the drift signal that the old "warning-saturated" verdict used to carry, now readable off a counter instead of a mood OR
...
The first three are _drift_ signals. The fourth is the risk signal, and it is bounded deliberately: its former blanket form ("novel/large/high-risk OR left debris") matched nearly every wave and produced 78 crew invocations across 20 waves. Debris is no longer a crew trigger at all — 2b-flags cleans self-caused debris in-wave and the rest batches. Thresholds tunable per project (`## Crew trigger + budget tuning`).

### Step 3: Crew pass (interim OR final)

**Interim passes are domain-matched, three agents maximum**, chosen by the diff's file globs. A corrective's re-crew is an interim pass and takes the same cap — its scope is narrower still (`## Corrective budget`). **Only the final pass, and the cleanup batch wave's pass, run all seven.** Invoke in parallel via Task tool, one call per agent, same message: `the-auditor` (a11y), `the-chemist` (test coverage), `the-chronicler` (doc drift), `the-diamantaire` (correctness), `the-ghostwriter` (voice on prose surfaces), `the-improver` (refactor opportunities), `the-stickler` (conventions) — each against the cumulative diff.
...
Write the gate artifact BEFORE looping back or resetting counters. Report all seven agents by name with each verdict, including the ones an interim pass did not run. Reset counters after a clean pass.
...
- Does NOT execute waves directly, and does NOT skip a crew pass whose trigger fired. `single-wave` sizing skips the crew _cadence_ up front; that is not a mid-run skip.
...
Crew-trigger defaults: every 4 waves OR 30 cumulative file changes, whichever first; interim passes 3 agents, final pass 7. Budget governor defaults: `max_correctives_per_wave=1`, `max_total_waves=25`, `max_corrective_waves=6`, `consecutive_no_progress=3`, `max_wave_retries=4`, `scaffolding_only_correctives=2`. Usage-window guard default: pause at `95%`.

Single canonical override block in the project CLAUDE.md:

```
## Loop de Looper
- crew-trigger: waves=N, files=M, interim-agents=N
...
Tighter triggers + budgets for high-drift domains (palette, auth surface) where churn signals a wrong approach early. Looser for long mechanical cleanup loops that legitimately run many waves. The severity floor and the admissibility test are NOT tunable — a project that wants a stricter bar tightens its own `CLAUDE.md` rules, which the crew already reads.
