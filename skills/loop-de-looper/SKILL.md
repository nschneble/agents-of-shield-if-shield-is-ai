...
- Does NOT treat a small direct edit as exempt from review when it touches security-relevant surface — user/network input landing in rendered or executed output needs a targeted `the-diamantaire` pass regardless of line count or timing (`## Corrective budget`).
