...
├── write run-state.json                      → first checkpoint; covers Steps 0-1 before any wave dispatches
...
**Then write `run-state.json` before dispatching wave 1** (atomic, `## State tracking`) — the queue, `sizing`, `nonbeliever` (verdict + notes), `required_not_loopable`, `last_crew_wave: 0`, every counter at zero, every wave `pending`. This is the run's first checkpoint and the only one that covers Steps 0-1. Until it lands, the nonbeliever verdict and the whole decomposition exist nowhere but context: a session that dies here has nothing to resume from and pays for both skills again, and a second nonbeliever pass on the same goal is free to size it differently. Observed — a run died after nonbeliever with no snapshot and re-ran Step 0 from scratch. The wave-boundary writes at 2c are what keep it current; this is what starts it.
...
- **`run-state.json`** — mutable position snapshot. Written once at the end of Step 1, before wave 1 dispatches, then rewritten after every wave and every crew pass. Source of truth for _where in the queue we are_.
...
- **Primary**: read `run-state.json`, then **audit it before trusting it** — `bash scripts/loop-state-audit.sh --branch <branch>`. Branch matches + file present is not enough: present-and-stale is the failure mode, and it looks identical to present-and-correct until something compares the snapshot to the append-only records beside it. Exit 0 → trust it for queue, counters, PR, last-crew. Exit 1 → the records win on every field the audit named; repair the snapshot from them, then continue. Exit 2 → the audit could not settle everything, so treat the unsettled fields as unknown and re-derive those by hand rather than assuming they are fine. Read the arms it printed, not just the code — it says which side of each disagreement it believes and why.
