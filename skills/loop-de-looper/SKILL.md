...
**Brief authoring on an EXTRACTION / SPLIT / god-file-refactor wave — cap EACH resulting file, or the bloat just relocates.** A wave whose whole point is to shrink an over-signal file (or extract a unit for testability) must set an explicit per-OUTPUT-file size ceiling in the brief, and require the executor to state the per-file code-LOC count in its hand-back. Without it, the extraction moves lines across a file boundary and the size signal silently re-trips on the NEW file — invisible to tsc and tests, caught only by a convention reviewer, which costs a corrective split plus a re-crew. Standing line for these waves: "each resulting file must be under ~100 CODE LOC (comments/blank excluded); report the per-file code-LOC count." Observed: a wave thinned a 229-line hook to 70 but created a 138-code-LOC extracted module; the final crew flagged it as a BLOCKER and a corrective wave re-split it into three under-100 files. This is the acceptance criterion the wave's success depends on, so name it in the brief, not just "do it alone".
...
4. **A PR state the loop did NOT itself set is a USER edit — surface it, NEVER revert it.** If finalization finds the PR already flipped to ready (when the loop created it draft), that is almost certainly the owner acting on their own PR, not a tool bug. Do NOT `gh pr ready --undo` it back to draft, and do NOT "restore" any state the loop didn't set. The loop's own components never flip draft→ready (`looper-commit` `## What looper-commit does NOT do`), so a ready PR at finalization was NOT the loop; treat a mismatch between the PR's live state and what the loop last set as the user, and leave it. Reverting the owner's decision while calling it "their call" is the exact inversion of respecting it (observed: a run found #105 flipped-to-ready + body-rewritten by the user, wrongly un-readied AND overwrote the body). If the true cause is ever genuinely a tool bug, surface it for the user to decide — do not self-correct external state.
...
- **User-edit guard (check FIRST, before composing anything).** The body may have been edited by the USER since the loop wrote it at wave 1. Before refreshing, read the current body (`gh pr view <N> --json body`) and compare it to what the loop last set. If it CHANGED, the user rewrote it — do NOT replace it with a generated recap. Two allowed moves only: (a) leave it entirely, or (b) AUGMENT it additively — append genuinely-new items into the sections the user already wrote (e.g. drop a real fix into their `**Fixed:**` list), leaving every line they authored byte-identical, matching their structure. The distinction is augment vs overwrite, never touch vs don't-touch. A wholesale `gh pr edit --body` that discards the user's version is the failure this guard exists to stop (observed: a terminal refresh clobbered a user-rewritten body). If a prior refresh already clobbered it, the exact prior text is recoverable via GraphQL `repository.pullRequest.userContentEdits(first:N){nodes{editedAt diff}}` (each node's `diff` is that revision's full body, newest first) — restore the user's, then augment.
