...
- **UI-glob gate.** Reuse the EXISTING UI-glob detector (`## Protocol` 2b) — do NOT define a second glob. Whole-run diff matched the UI-glob → INCLUDE the before/after ASCII wireframe + a11y call-out block; no match → omit it.
...
- Does NOT invent diff facts in the structured-recap PR-body refresh. Every block — tree, hunk, wireframe — is mechanically derived from the real whole-run diff (`git diff <wave1>^..HEAD`); anything beyond the diff is marked `inferred:`, secrets redacted, per `looper-commit`'s `## Structured recap (PR-body section)` grounding + secret-redaction rule. Same `ran: false`-style honesty the recap owes: no invented tree, hunk, or wireframe (`## Protocol` Step 4).
