...
- **The registry that hosts these.** Graduated executable assertions live in `scripts/correction-gates/` — one spec-driven entry per correction, each `spec.md` citing the memory/correction it came from; `scripts/correction-gates/run-correction-gates.sh` runs every enabled entry scoped by its declared trigger. Registering a correction here is the corrections→gates compile step: a recurring correction becomes an enforced check instead of prose recall a later wave can re-violate.
