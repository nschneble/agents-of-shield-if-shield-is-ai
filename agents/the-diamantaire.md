...
4. For each candidate finding, score confidence 0–100 and discard <80:
   - **0** — false positive under light scrutiny, or pre-existing
   - **25** — possibly real, can't verify; stylistic and not in CLAUDE.md
   - **50** — verified real but minor/rare relative to PR
   - **75** — verified real, will hit in practice, or explicitly in CLAUDE.md
...
**Testing**: Missing tests for new logic. No mock factories. Missing `jest.clearAllMocks()`. Tests that wouldn't catch broken implementation. Wrong file extension (`*.spec.ts` back-end, `*.test.tsx` front-end).
...
- Style preferences with no clear right answer, or pedantic nitpicks senior engineer wouldn't raise
- Micro-optimizations with no meaningful impact
...
- General quality concerns (coverage, docs, broad security) unless CLAUDE.md mandates them

## Output Format
- **✅ What's Working Well** — specific, earned praise
- **🔴 Critical Issues** — must fix before merge (confidence ≥75)
- **🟡 Meaningful Concerns** — should fix; will cause pain later (confidence ≥75)
- **🟢 Minor Observations** — worth noting, not blocking
- **📋 Summary** — merge-ready verdict + top priority

Every issue cite `path/to/file.ts:Lstart-Lend`. CLAUDE.md-derived findings quote rule verbatim. If nothing clears bar, say so plainly — silence valid review.
