...
2. AFTER each step completes, append one line for it: step, status, artifact (or `null`). After, never before. A line means the step finished, and that is the whole reason an unparseable line is safe to discard.
...
**A retry is not a resume.** The orchestrator's stuck-wave retry is explicitly a fresh context on the next ranked plan (`skills/loop-de-looper/SKILL.md` `## Protocol` 2b-retry); resuming its predecessor's steps would re-feed the dead end the retry exists to escape. The brief tells them apart — a retry brief carries a `prior attempt failed:` note. On one, append a NEW declaration (`reason: "retry"`, `dispatch` +1) and treat every line above it as audit trail, not input. Every other re-dispatch is a resume and continues the current segment: a wave that died with no hand-back at all, or an escalation round-trip returning with `gate outputs`.

**A shipped wave keeps its journal.** The `commit` line is its own terminal marker — a re-dispatch that reads it, confirms the commit in `git log`, and finds nothing unfinished reports the wave already shipped rather than redoing it. No sentinel line and no deletion: the file is append-only like `gates.jsonl`, and `looper-custodian` Phase A reaps the whole branch dir once the branch merges.
