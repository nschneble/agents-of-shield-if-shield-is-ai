...
#   1. The ceiling is raised to give a normal Phase E room to finish so that
#      Phase F runs. How much room it buys is unmeasured — see the comment
#      on the export below, and decision 15.
...
# Give a backgrounded Phase E room before end-of-turn tears it down. The one
# sourced figure is the 2026-07-20 cron.log's "still running after 600s;
# terminating" — where a fan-out was KILLED, not how long one takes — so the
# margin is unmeasured (decision 15). Left alone until a live Monday measures
# it. A ceiling-kill is handled below (resume), not retried.
